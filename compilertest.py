...
from constants import UNDEFINED, DEFAULT_OUTPUT_EXTENSION
...
def parseCommandLineArgs():
    ...
    parser = argparse.ArgumentParser(prog="Compiler.py",
                                     description="Capua Compiler Version {}".format(__version__,),
    ...
    args.output = args.output + args.extension if args.output.split(".")[-1] != "o" else args.output
...
if __name__ == '__main__':
    ...
    print("Compiler about to begin, following options will be used")
    print("  input file:             {}".format(usableArgs.input,))
    print("  output file:            {}".format(usableArgs.output,))

    compiler = Compiler(usableArgs.input, usableArgs.output)
    if os.path.exists(usableArgs.output):
        ...
        raise ValueError("An unknown error occurred while compiling the input file. Please validate "
